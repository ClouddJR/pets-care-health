...
import com.clouddroid.petscarehealth.utils.DateUtils.formatTime
import com.clouddroid.petscarehealth.utils.RemindersUtils
import com.clouddroid.petscarehealth.utils.RemindersUtils.deleteReminder
...
class EditReminderDialog(private val passedContext: Context, themeResId: Int) : Dialog(passedContext, themeResId) {
    ...
    private fun generateReminderForRepetitiveEvents(reminderKey: String): Reminder {
        return Reminder(reminderKey, titleEditText.text.toString(), chosenYear, chosenMonth, chosenDay, chosenHour, chosenMinute,
                (daysSpinner.getChildAt(0) as TextView).text.toString().toInt(), getIntervalsType(),
                passedAnimal?.name ?: "")
    }

    private fun getIntervalsType(): String {
        return when (intervalsSpinner.selectedItemId) {
            0.toLong() -> "days"
            1.toLong() -> "weeks"
            2.toLong() -> "months"
            else -> ""
