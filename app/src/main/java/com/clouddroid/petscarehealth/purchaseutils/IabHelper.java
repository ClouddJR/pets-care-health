package com.clouddroid.petscarehealth.purchaseutils;
...
public class IabHelper {
  ...
  public void startSetup(final OnIabSetupFinishedListener listener) {
    ...
    mServiceConn = new ServiceConnection() {
      ...
      public void onServiceConnected(ComponentName name, IBinder service) {
        ...
        if (listener != null) {
          listener.onIabSetupFinished(
              new IabResult(BILLING_RESPONSE_RESULT_OK,
                  "Setup successful."));
        }
      }
    };
  ...
  public boolean handleActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    if (resultCode == Activity.RESULT_OK && responseCode == BILLING_RESPONSE_RESULT_OK) {
      ...
      try {
        ...
        // Verify signature
        if (!Security
            .verifyPurchase(mSignatureBase64, purchaseData, dataSignature)) {
    ...
    (new Thread(new Runnable() {
      public void run() {
        IabResult result = new IabResult(
            BILLING_RESPONSE_RESULT_OK, "Inventory refresh successful.");
        Inventory inv = null;
        try {
  ...
  int queryPurchases(Inventory inv, String itemType)
    ...
    do {
      ...
      for (int i = 0; i < purchaseDataList.size(); ++i) {
        ...
        if (Security
            .verifyPurchase(mSignatureBase64, purchaseData, signature)) {
          logDebug("Sku is owned: " + sku);
          Purchase purchase = new Purchase(
              itemType, purchaseData, signature);
  ...
  int querySkuDetails(String itemType, Inventory inv,
    ...
    for (ArrayList<String> skuPartList : packs) {
      ...
      for (String thisResponse : responseList) {
        SkuDetails d = new SkuDetails(
            itemType, thisResponse);
